...
int main(int argc, char *argv[]) {
   ...
   d:	57                   	push   %edi
   e:	56                   	push   %esi
   f:	8d bd e8 f7 ff ff    	lea    -0x818(%ebp),%edi
  15:	8d b5 e8 f3 ff ff    	lea    -0xc18(%ebp),%esi
  1b:	53                   	push   %ebx
  1c:	51                   	push   %ecx
  1d:	81 ec 18 0c 00 00    	sub    $0xc18,%esp
     int start_time, end_time;
    start_time = uptime();
  23:	e8 53 04 00 00       	call   47b <uptime>
  ...
  28:	31 c9                	xor    %ecx,%ecx
    start_time = uptime();
  2a:	89 85 e4 f3 ff ff    	mov    %eax,-0xc1c(%ebp)
        for (j = 0; j < N; j++) {
  30:	89 ca                	mov    %ecx,%edx
  32:	31 c0                	xor    %eax,%eax
  34:	c1 e2 06             	shl    $0x6,%edx
  37:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  3e:	66 90                	xchg   %ax,%ax
            A[i][j] = i + j;
  40:	8d 1c 08             	lea    (%eax,%ecx,1),%ebx
  43:	89 1c 16             	mov    %ebx,(%esi,%edx,1)
            B[i][j] = i - j;
  46:	89 cb                	mov    %ecx,%ebx
  48:	29 c3                	sub    %eax,%ebx
        for (j = 0; j < N; j++) {
  4a:	83 c0 01             	add    $0x1,%eax
            B[i][j] = i - j;
  4d:	89 1c 17             	mov    %ebx,(%edi,%edx,1)
        for (j = 0; j < N; j++) {
  50:	83 c2 04             	add    $0x4,%edx
  53:	83 f8 10             	cmp    $0x10,%eax
  56:	75 e8                	jne    40 <main+0x40>
    for (i = 0; i < N; i++) {
  58:	83 c1 01             	add    $0x1,%ecx
  5b:	83 f9 10             	cmp    $0x10,%ecx
  ...
  63:	8d 9d e8 fb ff ff    	lea    -0x418(%ebp),%ebx
  69:	53                   	push   %ebx
  6a:	57                   	push   %edi
  6b:	56                   	push   %esi
  6c:	e8 2f 00 00 00       	call   a0 <matrix_multiply>
    //printf(1, "2\n");

    // Print the result
    print_matrix(C);
  71:	89 1c 24             	mov    %ebx,(%esp)
  74:	e8 b7 00 00 00       	call   130 <print_matrix>
    //printf(1, "3\n");
end_time = uptime();
  79:	e8 fd 03 00 00       	call   47b <uptime>
    printf(1, "Time taken: %d ticks\n", end_time - start_time);
  7e:	83 c4 0c             	add    $0xc,%esp
  81:	2b 85 e4 f3 ff ff    	sub    -0xc1c(%ebp),%eax
  87:	50                   	push   %eax
  88:	68 7c 08 00 00       	push   $0x87c
  8d:	6a 01                	push   $0x1
  8f:	e8 bc 04 00 00       	call   550 <printf>
    exit();
  94:	e8 4a 03 00 00       	call   3e3 <exit>
  99:	66 90                	xchg   %ax,%ax
  9b:	66 90                	xchg   %ax,%ax
  9d:	66 90                	xchg   %ax,%ax
  9f:	90                   	nop

000000a0 <matrix_multiply>:
void matrix_multiply(int A[N][N], int B[N][N], int C[N][N]) {
  ...
  af:	8b 75 10             	mov    0x10(%ebp),%esi
  b2:	89 7d f0             	mov    %edi,-0x10(%ebp)
  b5:	81 c7 00 04 00 00    	add    $0x400,%edi
  bb:	89 7d e4             	mov    %edi,-0x1c(%ebp)
  be:	8d b8 00 04 00 00    	lea    0x400(%eax),%edi
  c4:	05 40 04 00 00       	add    $0x440,%eax
  c9:	89 75 e8             	mov    %esi,-0x18(%ebp)
  cc:	89 7d e0             	mov    %edi,-0x20(%ebp)
  cf:	89 45 ec             	mov    %eax,-0x14(%ebp)
        for (j = 0; j < N; j++) {
  d2:	8b 7d e0             	mov    -0x20(%ebp),%edi
void matrix_multiply(int A[N][N], int B[N][N], int C[N][N]) {
  d5:	8b 75 e8             	mov    -0x18(%ebp),%esi
  d8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  df:	90                   	nop
            C[i][j] = 0;
  e0:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
  e6:	8b 5d f0             	mov    -0x10(%ebp),%ebx
  e9:	8d 87 00 fc ff ff    	lea    -0x400(%edi),%eax
  ef:	31 c9                	xor    %ecx,%ecx
  f1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
                C[i][j] += A[i][k] * B[k][j];
  f8:	8b 13                	mov    (%ebx),%edx
  fa:	0f af 10             	imul   (%eax),%edx
            for (k = 0; k < N; k++) {
  fd:	83 c0 40             	add    $0x40,%eax
 100:	83 c3 04             	add    $0x4,%ebx
                C[i][j] += A[i][k] * B[k][j];
 103:	01 d1                	add    %edx,%ecx
 105:	89 0e                	mov    %ecx,(%esi)
            for (k = 0; k < N; k++) {
 107:	39 f8                	cmp    %edi,%eax
 109:	75 ed                	jne    f8 <matrix_multiply+0x58>
        for (j = 0; j < N; j++) {
 10b:	83 c6 04             	add    $0x4,%esi
 10e:	8d 78 04             	lea    0x4(%eax),%edi
 111:	3b 7d ec             	cmp    -0x14(%ebp),%edi
 114:	75 ca                	jne    e0 <matrix_multiply+0x40>
    for (i = 0; i < N; i++) {
 116:	83 45 f0 40          	addl   $0x40,-0x10(%ebp)
 11a:	8b 45 f0             	mov    -0x10(%ebp),%eax
 11d:	83 45 e8 40          	addl   $0x40,-0x18(%ebp)
 121:	3b 45 e4             	cmp    -0x1c(%ebp),%eax
 124:	75 ac                	jne    d2 <matrix_multiply+0x32>
}
...
void print_matrix(int matrix[N][N]) {
 ...
 13c:	8d 77 40             	lea    0x40(%edi),%esi
 13f:	81 c7 40 04 00 00    	add    $0x440,%edi
 145:	8d 76 00             	lea    0x0(%esi),%esi
        for (j = 0; j < N; j++) {
 148:	8d 5e c0             	lea    -0x40(%esi),%ebx
 ...
 155:	83 c3 04             	add    $0x4,%ebx
            printf(1, "%d ", matrix[i][j]);
 158:	68 78 08 00 00       	push   $0x878
 15d:	6a 01                	push   $0x1
 15f:	e8 ec 03 00 00       	call   550 <printf>
        for (j = 0; j < N; j++) {
 164:	83 c4 10             	add    $0x10,%esp
 167:	39 f3                	cmp    %esi,%ebx
 169:	75 e5                	jne    150 <print_matrix+0x20>
        printf(1, "\n");
 16b:	83 ec 08             	sub    $0x8,%esp
    for (i = 0; i < N; i++) {
 16e:	8d 73 40             	lea    0x40(%ebx),%esi
        printf(1, "\n");
 171:	68 90 08 00 00       	push   $0x890
 176:	6a 01                	push   $0x1
 178:	e8 d3 03 00 00       	call   550 <printf>
    for (i = 0; i < N; i++) {
 17d:	83 c4 10             	add    $0x10,%esp
 180:	39 fe                	cmp    %edi,%esi
 182:	75 c4                	jne    148 <print_matrix+0x18>
}
...
sys_ps:
    movl SYS_ps, %eax
 48b:	a1 16 00 00 00       	mov    0x16,%eax
    int $64
 490:	cd 40                	int    $0x40
    ret
 492:	c3                   	ret    
 ...
 4ac:	89 d1                	mov    %edx,%ecx
{
 ...
 4d9:	0f b6 92 f4 08 00 00 	movzbl 0x8f4(%edx),%edx
 ...
 559:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 5c9:	ff 24 85 9c 08 00 00 	jmp    *0x89c(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 6d7:	90                   	nop
          s = "(null)";
 6d8:	ba 92 08 00 00       	mov    $0x892,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 6f1:	a1 04 0c 00 00       	mov    0xc04,%eax
{
 ...
 731:	5b                   	pop    %ebx
  freep = p;
 732:	89 15 04 0c 00 00    	mov    %edx,0xc04
}
 738:	5e                   	pop    %esi
 739:	5f                   	pop    %edi
 73a:	5d                   	pop    %ebp
 ...
 769:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 76c:	89 15 04 0c 00 00    	mov    %edx,0xc04
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 789:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 78c:	8b 3d 04 0c 00 00    	mov    0xc04,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7cf:	73 4f                	jae    820 <malloc+0xa0>
    ...
    if(p == freep)
 7d1:	8b 3d 04 0c 00 00    	mov    0xc04,%edi
 ...
 7fa:	e8 f1 fe ff ff       	call   6f0 <free>
  return freep;
 7ff:	8b 15 04 0c 00 00    	mov    0xc04,%edx
 ...
 82c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 82f:	89 15 04 0c 00 00    	mov    %edx,0xc04
}
 ...
 83f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 840:	c7 05 04 0c 00 00 08 	movl   $0xc08,0xc04
 847:	0c 00 00 
    base.s.size = 0;
 84a:	bf 08 0c 00 00       	mov    $0xc08,%edi
    base.s.ptr = freep = prevp = &base;
 84f:	c7 05 08 0c 00 00 08 	movl   $0xc08,0xc08
 856:	0c 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 859:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 85b:	c7 05 0c 0c 00 00 00 	movl   $0x0,0xc0c
 862:	00 00 00 
    if(p->s.size >= nunits){
